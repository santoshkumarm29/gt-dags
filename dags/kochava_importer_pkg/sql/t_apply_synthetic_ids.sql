...
select a.*,
       CASE
        ...
        CASE
            ...
            THEN a.android_id
        END id2 from gsnmobile.ad_partner_installs_gamesnetwork_ods a;
...
--             WHEN a.platform ='android' or a.platform='amazon'
--             THEN a.android_id
--         END id2 from gsnmobile.ad_partner_installs_bitrhymes_ods a;
...
--             WHEN a.platform ='android' or a.platform='amazon'
--             THEN a.android_id
--         END id2 from gsnmobile.ad_partner_installs_idlegames_ods a;

select analyze_statistics('gsnmobile.ad_partner_installs_staging');
...
from gsnmobile.ad_partner_installs_staging
...
CREATE LOCAL TEMPORARY TABLE ad_partner_installs_temp_1 ON COMMIT PRESERVE ROWS DIRECT
    ...
    /*+ direct,LABEL ('airflow-ua-ua_master_main_dag.kochava_importer_subdag-t_apply_synthetic_ids_2') */
        ...
        FROM
             gsnmobile.ad_partner_installs_staging a
        LEFT JOIN
...
DELETE /*+ direct, LABEL ('airflow-ua-ua_master_main_dag.kochava_importer_subdag-t_apply_synthetic_ids_4') */
 FROM gsnmobile.ad_partner_installs_raw
WHERE (ad_partner_id, app_name) IN
...
into gsnmobile.ad_partner_installs_raw
...
ALTER TABLE gsnmobile.ad_partner_installs, gsnmobile.ad_partner_installs_ods
RENAME TO ad_partner_installs_old, ad_partner_installs;

alter table gsnmobile.ad_partner_installs_old rename to ad_partner_installs_ods;
TRUNCATE TABLE gsnmobile.ad_partner_installs_ods;

-- unattributed installs
DELETE /*+ direct, LABEL ('airflow-ua-ua_master_main_dag.kochava_importer_subdag-t_apply_synthetic_ids_5') */
 FROM gsnmobile.kochava_unattributed_installs
WHERE (ad_partner_id, app_name) IN
      (SELECT ad_partner_id, app_name FROM ad_partner_installs_temp_2 WHERE upper(network_name) = 'UNATTRIBUTED');

insert /*+ direct, LABEL ('airflow-ua-ua_master_main_dag.kochava_importer_subdag-t_apply_synthetic_ids_5') */
into gsnmobile.kochava_unattributed_installs
select distinct ad_partner,
platform,
inserted_on,
bid_id,
...
from ad_partner_installs_temp_2 x WHERE upper(network_name) = 'UNATTRIBUTED';


delete /*+direct,LABEL ('airflow-ua-ua_master_main_dag.kochava_importer_subdag-t_apply_synthetic_ids_6')*/
from gsnmobile.kochava_unattributed_installs
WHERE (ad_partner_id, app_name) IN
      (SELECT DISTINCT ad_partner_id, app_name
         FROM ad_partner_installs_temp_2  WHERE upper(network_name) <> 'UNATTRIBUTED')
      and date(install_date) between date('{{ ds }}') - 30 and  date('{{ ds }}') + 7;

commit;
-- SELECT PURGE_TABLE('gsnmobile.ad_partner_installs');
