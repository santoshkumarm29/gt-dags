"""Feature queries for WorldWinner LTR based on Registration Date
"""
...
def get_features_df(install_start_date, install_end_date, n_days_observation, n_days_horizon, connection=None,
    ...
    feature_callables = [
        ...
        # get_email_info, #email is "purged" due to GDPR/CCPA.  Data is in another table, but I'm not sure Sony wants me to use the data.
        # get_seasonality_transformed, #too much python code, wasn't fruitful
        # get_chum_ftue, #no important features
        get_time_features,
        get_mobile_play,
        get_session_info,
        #get_sample_weights
...
def get_user_id_base(install_start_date, install_end_date, onehot_encode=False, connection=None,
    ...
    with users as (
      select user_id, createdate::date as install_date
      from ww.dim_users
    ...
    df = get_dataframe_from_query(query, index_col='user_id', connection=connection)
    return df
...
def get_iap_revenue(install_start_date, install_end_date, n_days=1, connection=None):
    ...
    raw_games_info as(
        select users.user_id, users.create_date, trans_date, amount
        from users
        left join games_played
        on users.user_id = games_played.user_id
        and datediff('day', create_date, trans_date) between 0 and ({n_days}-1)
    )
...
def get_retention_df(install_start_date, install_end_date, n_days_observation=1, n_days_horizon=30, connection=None):
    ...
    /* Get install cohort for WW LTV */
    with users as (
      select user_id, createdate::date as install_day
      from ww.dim_users
      where createdate::date between '{install_start_date}' and '{install_end_date}'
    ),
    days_played_retention as (
      ...
      from (
        ...
        where start_time::date between '{install_start_date}' and (date('{install_end_date}') + interval '{n_days_horizon} days')
      ) as pg
    )
        ...
        and datediff('day', a.install_day, b.trans_date) between {n_days_observation} and ({n_days_horizon}-1)
        group by 1
    """.format(install_start_date=install_start_date, install_end_date=install_end_date,
...
def get_days_played_pg(install_start_date, install_end_date, n_days=1, connection=None):
    ...
    days_played_info as(
        select users.user_id, users.install_day, trans_date
        from users
        left join days_played
        on users.user_id = days_played.user_id
        and datediff('day', install_day, trans_date) between 0 and ({n_days}-1)
    )
    select user_id, 
    count(trans_date) as pg_days_played,
    coalesce(datediff('day', install_day, min(trans_date)), -1) + 1 as pg_first_played,
    coalesce(datediff('day', install_day, max(trans_date)), -1) + 1 as pg_last_played
    from days_played_info
    group by user_id, install_day
    """.format(install_start_date=install_start_date, install_end_date=install_end_date, n_days=n_days)
    df = get_dataframe_from_query(query, index_col='user_id', connection=connection)
    return df
...
def get_days_played_it(install_start_date, install_end_date, n_days=1, connection=None):
    ...
    coalesce(datediff('day', install_day, min(trans_date)), -1) + 1 as it_first_played,
    coalesce(datediff('day', install_day, max(trans_date)), -1) + 1 as it_last_played
    from days_played_info
    group by user_id, install_day
    """.format(install_start_date=install_start_date, install_end_date=install_end_date, n_days=n_days)
    df = get_dataframe_from_query(query, index_col='user_id', connection=connection)
...
def get_sin_cos_install_time(install_start_date, install_end_date, n_days=1, connection=None):
    ...
    with users as (
      select user_id, createdate as created_at
      from ww.dim_users
      where createdate::date between '{install_start_date}' and '{install_end_date}'
    ),
    time_of_install_subquery as (
      select user_id, created_at, MOD((DATE_PART('SECOND', created_at) + DATE_PART('MINUTE', created_at) * (60) + DATE_PART('HOUR', created_at) * (60*60)), 60*60*24) as seconds_into_day
      from users
    )
...
# def _get_signups_winnings(install_start_date, install_end_date, n_days=1, connection=None):
#    """
...
#    )
#      select a.user_id,
#      COALESCE(SUM(b.amount), 0) as total_winnings,
#      COALESCE(SUM(c.amount), 0) as total_signups
#      from users a
#      left join game_winnings b
#      on a.user_id = b.user_id
#      and datediff('day', create_date, trans_date) between 0 and ({n_days}-1)
#      left join game_signups c
...
def get_free_paid_games(install_start_date, install_end_date, n_days=1, connection=None):
    ...
    games_played_info as(
        select users.user_id, users.create_date, trans_date, fee, purse
        from users
        left join games_played
        on users.user_id = games_played.user_id
        and datediff('day', create_date, trans_date) between 0 and ({n_days}-1)
...
# def _get_entry_sizes(install_start_date, install_end_date, n_days=1, connection=None):
#    """
#    https://bi.worldwinner.com/webreports/sqlide/ide#dynamic_param=true&guid=f18e3a4c-3f90-4472-bf70-29313d9644e1
#    """
#    query = """
#    /* Gets the number of free and paid games signed up for, and their fees */
#    with users as (
...
#    )
#      select user_id,
#      MAX(case when fee > 0 and purse > 0 then fee else 0 end) as max_entry_fee,
#      AVG(case when fee > 0 and purse > 0 then fee else 0 end) as avg_entry_fee
#      from games_played_info
#      group by user_id
#    """.format(install_start_date=install_start_date, install_end_date=install_end_date, n_days=n_days)
#    df = get_dataframe_from_query(query, index_col='user_id', connection=connection)
...
def get_favorite_game(install_start_date, install_end_date, n_days=1, connection=None):
    ...
    games_played_info as(
        select users.user_id, users.create_date, trans_date, gametype_id
        from users
        left join games_played
        on users.user_id = games_played.user_id
        and datediff('day', create_date, trans_date) between 0 and ({n_days}-1)
...
def get_total_net_winnings(install_start_date, install_end_date, n_days=1, connection=None):
    ...
    raw_transactions_info as(
        select users.user_id, users.create_date, trans_date, amount, trans_type
        from users
...
def get_first_last_balance(install_start_date, install_end_date, n_days=1, connection=None):
    ## TODO: let's add bonus bucks (deposit_credits) to this query as well.
    """
    ...
    raw_games_info as(
        select users.user_id, users.create_date, trans_time, trans_type, trans_date, balance, play_money,
        deposit_credits, deposit_credits_amount
        from users
        left join games_played
        on users.user_id = games_played.user_id
...
def get_is_banned(install_start_date, install_end_date, n_days=1, connection=None):
    ...
    with banned_df as (
      ...
      when datediff('day', createdate::date, user_banned_date::date) between 0 and ({n_days}-1) then 1
...
# def _get_email_info(install_start_date, install_end_date, n_days=1, connection=None, categorical_encode=True):
#    """
...
#      from ww.dim_users
#      where createdate::date >= '{install_start_date}'
#    ),
#    student_df as (
...
def get_user_info(install_start_date, install_end_date, n_days=1, connection=None):
    ...
    user_info as (
      select *
      from ww.dim_users
      where createdate between '{install_start_date}' and (date('{install_end_date}') + interval '{n_days} days' - interval '1 days')
    ...
    user_agg as (
      ...
      -- Banned --
      --coalesce((user_banned > 0 and datediff('second', create_date, user_banned_date) between 0 and (86400 * {n_days}))::INT, 0) as is_banned, -- captured in another feature query!
      LEAST(coalesce(datediff('second', create_date, user_banned_date)/(60*60), 24*{n_days}), 24*{n_days}) as time_to_ban_hr,
      -- Other --
    ...
    agg as (
      ...
      --max(real_money_date) as real_money_date, -- not used
...
def get_time_features(install_start_date, install_end_date, n_days=1, connection=None):
    ...
    DAYOFMONTH(create_date) as day_of_month,
    DAYOFYEAR(create_date) as day_of_year,
    WEEK_ISO(create_date) as iso_week,
    QUARTER(create_date) as quarter,
    DATEDIFF('day', '2008-01-01', create_date) as days_since_2008
    from users
    """.format(install_start_date=install_start_date, install_end_date=install_end_date, n_days=n_days)
    df = get_dataframe_from_query(query, index_col='user_id', connection=connection)
    return df
...
def get_mobile_play(install_start_date, install_end_date, n_days=1, connection=None):
    ...
    phoenix_events as (
      SELECT a.user_id, a.create_date, b.idfv, b.event_time
      FROM users a
      LEFT JOIN phoenix.events_client b
      ON a.user_id::INT = b.user_id::INT
      WHERE datediff('day', create_date, event_day) BETWEEN 0 AND ({n_days}-1)
    ),
    last_active_query AS (
      ...
      group by user_id, idfv, create_date
    ),
    joined_idfvs as (
      select a.user_id, a.create_date, b.idfv,  b.first_event_time, b.last_event_time
      from users a
    ...
    COALESCE(MIN(DATEDIFF('day', create_date, first_event_time)), -1) + 1 as first_mobile_event_time,
    COALESCE(MAX(DATEDIFF('day', create_date, last_event_time)), -1) + 1 as last_mobile_event_time
    from joined_idfvs
    where user_id IS NOT NULL
    group by 1
        """.format(install_start_date=install_start_date, install_end_date=install_end_date, n_days=n_days)
...
def get_entry_sizes(install_start_date, install_end_date, n_days=1, connection=None):
    ...
    user_unagg as (
      ...
      on a.user_id::int = b.user_id::int
      and datediff('day', create_date, start_time::date) BETWEEN 0 AND ({n_days}-1)
...
def get_games_played_by_id(install_start_date, install_end_date, n_days=1, connection=None):
    ...
    user_agg as (
      ...
      on a.user_id::int = b.user_id::int
      and datediff('day', create_date, start_date) BETWEEN 0 AND ({n_days}-1)
...
def get_chum_ftue(install_start_date, install_end_date, n_days=1, connection=None):
    ...
    with users as (
      select user_id, createdate as create_date
      from ww.dim_users
      where createdate::date between '{install_start_date}' and '{install_end_date}'
    ),
    chum_trigger_bb as (
      ...
      case when event_name = 'tutorialClickEvent' and str_attr1 = 'ftue_p1_results' then 1 else 0 end as ftue_p1_results,
    ...
    SUM(ftue_p1_results + ftue_p1_results_shown) as ftue_p1_events,
    SUM(ftue_p2_results + ftue_p2_warm_up_tournament) as ftue_p2_events,
    SUM(ftue_p3_results + ftue_p3_warm_up_tournament) as ftue_p3_events,
    SUM(ftue_tournament_results_button + ftue_tournament_screen1 + ftue_tournament_screen2) as ftue_tournament_events,
    SUM(welcome_back) as welcome_back,
    MAX(case when bonus_bucks_received > 0 then datediff('hour', create_date, event_time) else NULL end) as time_to_chum_hr
    from users a
    left join chum_trigger_bb b
    on a.user_id::int = b.user_id::int
    and datediff('day', create_date::date, event_time::date) between 0 and ({n_days}-1)
    group by a.user_id
    """.format(install_start_date=install_start_date, install_end_date=install_end_date, n_days=n_days)
    df = get_dataframe_from_query(query, index_col='user_id', connection=connection)
    return df
...
def get_session_info(install_start_date, install_end_date, n_days=1, connection=None):
    ...
    app_sessions as(
      ...
      from (
        ...
        and datediff('day', create_date::date, start_ts::date) between 0 and ({n_days}-1)
    ...
    web_sessions as(
      ...
      on a.user_id::int = b.user_id::int
      and datediff('day', create_date::date, start_time::date) between 0 and ({n_days}-1)
    ...
    datediff('hour', coalesce(last_app_session_start_ts, create_date::date), create_date::date + interval '{n_days} days') as hrs_since_last_app_session,
    ...
    datediff('hour', coalesce(last_web_session_start_ts, create_date::date), create_date::date + interval '{n_days} days') as hrs_since_last_web_session
    from users a
    left join app_sessions b
    on a.user_id::int = b.user_id::int
    and datediff('day', create_date::date, last_app_session_start_ts::date) between 0 and ({n_days}-1)
    left join web_sessions c
    on a.user_id::int = c.user_id::int
    and datediff('day', create_date::date, last_web_session_start_ts::date) between 0 and ({n_days}-1)  
    """.format(install_start_date=install_start_date, install_end_date=install_end_date, n_days=n_days)
    df = get_dataframe_from_query(query, index_col='user_id', connection=connection)
    return df