/*
Perform an upsert on target table for tables that have compound PRIMARY KEY containing 2-n columns
...
MERGE /*+ DIRECT,LABEL('airflow-{{ params.studio }}-{{ params.dag_id }}-{{ params.task_id }}') */
 ...
 USING (
  ...
  WITH dedupe AS (
    SELECT {{ column_names|join(', ') }},
           ...
           ROW_NUMBER() OVER(PARTITION BY {% for column_name in params.pk %} {{ column_name }}{% if not loop.last %},{% endif %}  {% endfor %} ORDER BY s3_key DESC ) AS row_num
 ...
 ON {% for column_name in params.pk %} source.{{ column_name }} = target.{{ column_name }}{% if not loop.last %} and {% endif %}{% endfor %}
