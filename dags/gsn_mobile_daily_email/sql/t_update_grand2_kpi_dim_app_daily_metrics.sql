...
/*+ direct,LABEL ('airflow-all-etl_gsn_daily_kpi_email-t_update_grand2_kpi_dim_app_daily_metrics')*/
(
select
distinct app, event_day, case when app = 'GSN Grand Casino' then synthetic_id else user_id end as user_id
from gsnmobile.events_dau
where app in ('GSN Grand Casino 2', 'GSN Grand Casino')
...
from gsnmobile.events_dau
where app in ('GSN Grand Casino 2')
and event_day between date('{{ ds }}') - cast('{{ params.days_back }}' as int) - 34 and date('{{ ds }}')
);
...
SELECT
      ...
      union
      select
      distinct 'GSN Grand Casino' as app,
...
insert  /*+ direct,LABEL ('airflow-all-etl_gsn_daily_kpi_email-t_update_grand2_kpi_dim_app_daily_metrics')*/ into kpi.dim_app_daily_metrics_historical(event_day,app_name,transactional_bookings,daily_active_users,monthly_active_users,installs,advertising_revenue,
