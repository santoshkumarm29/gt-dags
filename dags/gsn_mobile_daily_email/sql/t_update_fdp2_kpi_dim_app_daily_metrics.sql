...
/*+ direct,LABEL ('airflow-all-etl_gsn_daily_kpi_email-t_update_blackdeck_kpi_dim_app_daily_metrics')*/
(
select
distinct app, event_day, user_id
...
SELECT
      ...
      WHERE timestampadd('mi', -5, created_at) <= first_created_at -- filters users migrated from fdp1
...
insert  /*+ direct,LABEL ('airflow-all-etl_gsn_daily_kpi_email-t_update_blackdeck_kpi_dim_app_daily_metrics')*/ into kpi.dim_app_daily_metrics_historical(event_day,app_name,transactional_bookings,daily_active_users,monthly_active_users,installs,advertising_revenue,
