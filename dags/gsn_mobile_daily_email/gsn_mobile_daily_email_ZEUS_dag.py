...
default_args = {
    ...
    'start_date': datetime(2021, 12, 1) if is_airflow() else datetime(2021, 12, 1),
...
dag = DAG(
    dag_id='etl_gsn_daily_kpi_email_001',
...
t_final_send_email = PythonOperator(
    ...
    op_kwargs={'vertica_conn_id': 'vertica_conn',
               'receivers': ('games-all@gsngames.com',),
               'subject': 'Daily KPIs: {rev} ({delta_rev}) IAP Rev | {dau} ({delta_dau}) DAU | {installs} ({delta_installs}) Installs',
               'app_dict_list': app_dict_list,
               'subject_key_name': 'GSN Games',
               'template_path': join(dirname(__file__), 'templates/daily_email_zeus.html'),
               'ads_data': True,
...
t_preview_send_email = PythonOperator(
    ...
    op_kwargs={'vertica_conn_id': 'vertica_conn',
               ...
               'subject': 'ZEUS: PREVIEW: Daily KPIs: {rev} ({delta_rev}) IAP Rev | {dau} ({delta_dau}) DAU | {installs} ({delta_installs}) Installs',
               'app_dict_list': app_dict_list,
               'subject_key_name': 'GSN Games',
